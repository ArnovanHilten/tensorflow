...
class CustomConnected1DLayersTest(test.TestCase, parameterized.TestCase):

  @parameterized.parameters(_DATA_FORMAT_PADDING_IMPLEMENTATION)
  def test_CustomConnected_1d(self, data_format, padding, implementation):
    with self.cached_session():
      ...
      for strides in [1]:
        ...
        if padding == 'same' and implementation == 1:
          self.assertRaises(ValueError, keras.layers.CustomConnected1D,
                            **kwargs)
        else:
          testing_utils.layer_test(
              keras.layers.CustomConnected1D,
              kwargs=kwargs,
              input_shape=(num_samples, num_steps, input_dim))

  @parameterized.parameters(_DATA_FORMAT_PADDING_IMPLEMENTATION)
  def test_CustomConnected_1d_regularization(self, data_format, padding,
                                              implementation):
    num_samples = 2
    num_steps = 8
    input_dim = 5
    ...
    if padding == 'same' and implementation == 1:
      self.assertRaises(ValueError, keras.layers.CustomConnected1D, **kwargs)
    else:
      with self.cached_session():
        layer = keras.layers.CustomConnected1D(**kwargs)
...
@combinations.generate(combinations.combine(mode=['graph', 'eager']))
class CustomConnected2DLayersTest(test.TestCase, parameterized.TestCase):

  @parameterized.parameters(_DATA_FORMAT_PADDING_IMPLEMENTATION)
  def test_CustomConnected_2d(self, data_format, padding, implementation):
    with self.cached_session():
      num_samples = 8
      filters = 3
      stack_size = 4
      ...
      for strides in [(1, 1), (2, 2)]:
        ...
        if padding == 'same' and implementation == 1:
          self.assertRaises(ValueError, keras.layers.CustomConnected2D,
                            **kwargs)
        else:
          testing_utils.layer_test(
              keras.layers.CustomConnected2D,
              kwargs=kwargs,
              input_shape=(num_samples, num_row, num_col, stack_size))

  @parameterized.parameters(_DATA_FORMAT_PADDING_IMPLEMENTATION)
  def test_CustomConnected_2d_channels_first(self, data_format, padding,
                                              implementation):
    with self.cached_session():
      num_samples = 8
      filters = 3
      ...
      if padding == 'same' and implementation == 1:
        self.assertRaises(ValueError, keras.layers.CustomConnected2D, **kwargs)
      else:
        testing_utils.layer_test(
            keras.layers.CustomConnected2D,
            kwargs=kwargs,
  ...
  @parameterized.parameters(_DATA_FORMAT_PADDING_IMPLEMENTATION)
  def test_CustomConnected_2d_regularization(self, data_format, padding,
                                              implementation):
    num_samples = 2
    filters = 3
    stack_size = 4
    ...
    if padding == 'same' and implementation == 1:
      ...
      with self.cached_session():
        layer = keras.layers.CustomConnected2D(**kwargs)
...
@combinations.generate(combinations.combine(mode=['graph', 'eager']))
class CustomConnectedImplementationModeTest(test.TestCase,
  ...
  def test_CustomConnected_implementation(self, width, data_format):
    with self.cached_session():
      num_samples = 4
      num_classes = 3
      num_epochs = 2
...
def get_model(implementation,
  ...
  if len(kernel_size) == 1:
    lc_layer = keras.layers.CustomConnected1D
  elif len(kernel_size) == 2:
    lc_layer = keras.layers.CustomConnected2D
  else:
...
def copy_lc_weights_2_to_1(lc_layer_2_from, lc_layer_1_to):
  ...
  if data_format == 'channels_first':
    if isinstance(lc_layer_2_from, keras.layers.CustomConnected1D):
      permutation = (3, 0, 1, 2)
    elif isinstance(lc_layer_2_from, keras.layers.CustomConnected2D):
...
def copy_model_weights(model_from, model_to):
  for l in range(len(model_from.layers)):
    ...
    if (isinstance(
        layer_from,
        (keras.layers.CustomConnected2D, keras.layers.CustomConnected1D)) and
        isinstance(layer_to, (keras.layers.CustomConnected2D,
                              keras.layers.CustomConnected1D))):
